...
- **現在のテストブランチ**: `codex/email-validator-hotfix`
- **本番ブランチへのマージ条件**: テスト完了後、ユーザーが明示的に同意した場合のみマージする。

## ブランチ運用ルール
- `codex/email-validator-hotfix` を現在のテストブランチとして扱う。
- 開発・修正・検証はテストブランチ上で行う。
- テストが通っても、ユーザーの明示的な承認なしに本番ブランチへ merge しない。
- 本番ブランチへ merge する前に、実行した検証コマンドと結果をユーザーへ報告する。
